  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50 font-sans selection:bg-emerald-500/30">
      <div className="max-w-4xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
        ...
        {/* Results */}
        <AnimatePresence>
          {isOutOfScope && !isLoading && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 flex flex-col items-center text-center gap-4"
            >
              <div className="p-3 bg-amber-500/20 rounded-full">
                <AlertCircle className="w-8 h-8 text-amber-400" />
              </div>
              <div>
                <h3 className="text-xl font-semibold text-amber-400 mb-2">Out of Scope</h3>
                <p className="text-zinc-400">
                  This doesn't look like a startup idea or business pitch. Please provide a concept related to a product, service, or business venture for analysis.
                </p>
              </div>
            </motion.div>
          )}

          {analysis && !isLoading && !isOutOfScope && (
            <motion.div
